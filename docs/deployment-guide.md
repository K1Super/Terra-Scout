...
> 一键完成「后端门禁+打包 → JRE 裁剪 → 前端构建 → NSIS 安装包」可执行仓库根 `scripts/build.ps1`；仅跑质量门禁（后端测试 + 前端类型检查/单测）执行 `scripts/test.ps1`。
...
- 卸载时弹出选择框（默认「是」= 删除全部数据），确认后一并删除 `%USERPROFILE%\.terrascout` 数据目录与 Electron 用户数据目录（`%APPDATA%\terra-scout-electron`）。
- 选择「否」则保留用户数据（仅卸载程序本体与安装目录）。
- 项目 `.devenv` 不删除（属于用户项目）。
- 删除失败（文件被占用等）不阻断卸载，残留内容需手动清理。
