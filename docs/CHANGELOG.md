...
- **R52（AI 标准配置模板 + 卸载彻底清理）**：设置页 AI 辅助升级为标准配置模板——`deepseek` / `glm` / `openai-compatible` 三套模板（默认接入地址 + 推荐模型，可覆盖，不覆盖用户自定义地址），设置新增 `aiProvider / aiBaseUrl / aiApiKey / aiModel` 白名单键与持久化（非法供应商回落默认、baseUrl 去尾部斜杠）；新增 `POST /api/v1/settings/ai/test` 真实连通性探测（OpenAI 兼容最小请求，5s 连接 / 10s 总超时，`{ok, message, latencyMs}`；不完整或非法配置 → 新错误码 `400010`，错误码总量 46→47）；诊断包与 info 摘要对 `aiApiKey` 掩码（保留首尾 4 位）。卸载增强：NSIS 卸载弹窗（默认「是」= 删除全部）可一键清理 `%USERPROFILE%\.terrascout` 数据目录与 Electron 用户数据目录 `%APPDATA%\terra-scout-electron`，选「否」保留数据。项目构建/门禁/联调脚本统一至仓库根 `scripts/`（`build.ps1` 一键生产构建、`build-jre.ps1` JRE 裁剪、`test.ps1` 全量门禁、`dev.ps1`+`dev.bat` 本机联调），原 `terra-scout-electron/scripts/` 四个脚本删除。
