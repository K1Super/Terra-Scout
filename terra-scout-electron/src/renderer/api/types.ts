/**
 * 与接口契约对齐的 TS 类型（统一响应 code==HTTP×1000）。
 */
...
export interface ApiEnvelope<T> {
  ...
  traceId?: string;
  timestamp?: number;
  /** 结构化明细（ErrorResponse.details），含 hint 时前端附加展示。 */
...
export interface SdkInstallPlan {
  ...
  reason?: string;
  /** 可选版本候选表，供详情页下拉选配；无候选（匹配失败被跳过）时不渲染选择器。 */
...
/** 用户 SDK 版本选配：{language, version} 覆盖自动匹配结果。 */
...
export interface SdkInstallRequest {
  ...
  projectId?: string;
  idempotencyKey: string;
  /** 可选：自定义安装根目录（绝对路径），自动落位 {所选目录}\{语言小写}\{版本}；缺省标准仓库。 */
  installDir?: string;
}
